        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-12 col-md-4 d-flex align-items-center">
                <form method="get" action="{{ route('products.index') }}" id="categoryForm" class="d-flex w-100">
                    <select id="category-filter" name="category" class="form-control form-control-sm w-100"
                            aria-controls="DataTables_Table_0">
                        <option value="">Select Category</option>
                        @foreach($categories as $category)
                            <option
                                value="{{ $category->id }}" {{ Request::input('category') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-sm-12 col-md-4 d-flex align-items-center">
                <form method="get" action="{{ route('products.index') }}" id="searchForm" class="d-flex w-100">
                    <input type="search" name="code" id="code" class="form-control form-control-sm w-100"
                           placeholder="Code / Nom / Description" value="{{ request()->input('code') }}"
                           aria-controls="DataTables_Table_0">
        ...
        <ul class="nav nav-tabs" id="product-tabs" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active" id="list-tab" data-bs-toggle="tab" href="#list" role="tab"
                   aria-controls="list" aria-selected="true">Liste</a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="grid-tab" data-bs-toggle="tab" href="#grid" role="tab" aria-controls="grid"
                   aria-selected="false">Grille</a>
            </li>
        </ul>
                        ...
                        <table
                            ...
                            @forelse ($products as $product)
                                <tr role="row" class="odd">
                                    ...
                                    <td>
                                        <img width="50px" height="50px" src="{{ asset('storage/' . $product->image) }}"
                                             alt="Product Image" class="rounded-circle">
                                    </td>
                                    <td><a href="javascript: void(0);"
                                           class="text-dark fw-bold">{{ $product->name }}</a></td>
                                    ...
                                    <td>
                                        @if ($product->category)
                                            <a href="{{ route('categories.show', $product->category_id) }}"
                                               class="btn btn-link">
                                                <span class="badge"
                                                      style="background-color:#F97300;">{{ $product->category->name }}</span>
                                            </a>
                                        @else
                                            ----------
                                        @endif
                                ...
                                <div class="card-body">
                                    <form method="POST" action="{{ route('createInvoice') }}">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <strong id="cart-total" class="badge"
                                                    style="background-color:#790cdf; font-size:16;">Total: 0.00
                                                MAD</strong>
                                        </div>
                        ...
                        @foreach ($products as $product)
                            <div class="col">
                                ...
                                <span style="font-size: 0.85rem;">
                                    <span
                                        class="badge bg-success">Stock: {{ $product->stock ? $product->stock->quantity : 'N/A' }}</span>
                                </span>
                                            <span style="font-size: 0.85rem;">
