...
<!-- Champ NFact en lecture seule -->
            ...
            <tbody id="sales-table-body" >
                @if ($isUpdate)
                    ...
                    <tr id="sale-row-template">
                        ...
                        <td>
                            <input type="number" name="unit_price[]" class="form-control unit-price" value="{{ $detail->unit_price }}">
                        </td>
                        <td>
                    ...
                    <tr id="sale-row-template">
                        <td>
                            <select name="product_id[]" class="form-select product-select">
                                <option value="">Select Product</option>
                                @foreach($products as $product)
                                <option value="{{ $product->id }}" data-price="{{ $product->priceV }}">{{ $product->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <input type="number" name="quantity[]" class="form-control quantity"  value="0">
                        </td>
                        <td>
                            <input type="number" name="unit_price[]" class="form-control unit-price" value="0">
                        </td>
        ...
        <div class="form-group">
            ...
            <input type="number" name="tremise" id="tremise" class="form-control" value="{{ old('tremise', $sale->tremise) }}">
        </div>

        <div class="form-group">
            <label for="mremise" class="form-label">Remise</label>
            <input type="number" name="mremise" id="mremise" class="form-control" value="{{ old('mremise', $sale->mremise) }}">
        </div>
...
$('#add-sale-row').click(function() {
    var newRow = $('#sale-row-template').clone().removeAttr('id').removeAttr('style');
    $('#sales-table-body').append(newRow);
    resetRow(newRow);
});

// Supprimer la dernière ligne de vente
$('#remove-sale-row').click(function() {
    var rowCount = $('#sales-table-body tr').length;
    if (rowCount > 1) {
        $('#sales-table-body tr:last').remove();
        calculateSummary();
    } else {
