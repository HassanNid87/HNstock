            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div class="d-inline-flex w-50">
                    <form method="get" id="stockForm" class="d-inline-flex align-items-center w-100">
                        <div class="me-2 w-50">
                            <select id="category-filter" name="category" class="form-control form-control-sm"
                        ...
                        <div class="w-50">
                            <input type="search" name="name" id="name-filter" class="form-control form-control-sm"
                                   style="height: 100%;"
                                   placeholder="CodeBarre / Name / Description" value="{{ Request::input('name') }}"
                                   aria-controls="DataTables_Table_0">
                        </div>
                    </form>
                </div>
                            ...
                            @forelse ($stocks as $stock)
                                <tr role="row" class="odd">
                                    <td class="text-center">
                                        <input type="checkbox" class="form-check-input" id="stockcheck{{ $stock->id }}"
                                               value="{{ $stock->id }}">
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td>
                                        @if ($stock->product && !$stock->product->category->isDefault)
                                            <a href="{{ route('categories.show', $stock->product->category->id) }}"
                                               class="btn btn-link">
