                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <form method="get" id="dateFilterForm" action="{{ url()->current() }}"
                            ...
                            <div class="input-daterange input-group w-100" id="datepicker6"
                            ...
                            <div class="col-md-6">
                                <form method="get" id="nameFilterForm" class="d-flex align-items-center">
                                    <input type="search" name="name" id="name-filter"
                                           class="form-control form-control-sm w-100"
                                           placeholder="CodeBarre / Name / Description"
                                           value="{{ Request::input('name') }}" aria-controls="DataTables_Table_0">
                                </form>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="table table-centered datatable dt-responsive nowrap table-card-list dataTable no-footer dtr-inline"
                                ...
                                @foreach ($stockOuts as $stockOut)
                                    <tr data-category-id="{{ $stockOut['product']->category->id }}">
                                        ...
                                        <td class="text-center">
                                            @if ($stockOut['product'] && $stockOut['product']->image)
                                                <img width="50px" height="50px"
                                                     src="{{ asset('storage/' . $stockOut['product']->image) }}"
                                                     alt="Product Image" class="rounded-circle">
                                            @else
                                                <img width="50px" height="50px"
                                                     src="{{ asset('storage/default-image.png') }}" alt="Default Image"
                                        ...
                                        <td><span class="badge"
                                                  style="background-color:#F97300;">{{ $stockOut['product']->category->name ?? 'Non spécifié' }}</span>
                                        </td>
                                        <td><span class="badge bg-success"
                                                  style="font-size: 12px;">{{ $stockOut['quantity'] }}</span></td>
                                        <td>{{ $stockOut['date'] }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
